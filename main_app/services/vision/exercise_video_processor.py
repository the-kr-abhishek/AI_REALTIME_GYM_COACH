...
class VideoProcessorClass(VideoProcessorBase):
    def __init__(self):
        ...
        self._exercise_type = "Squats"    # DEFAULT
    ...
    def recv(self, frame):
        image = np.asarray(
            cv2.flip(frame.to_ndarray(format="bgr24"), 1),    # image will be flipped from real one like in mirror
            dtype=np.uint8
        )

        mp_image = mp.Image(                                  # detector expects a MediaPipe Image
