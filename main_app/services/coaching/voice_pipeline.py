...
class VoicePipeline:
    ...
    def process_event(self, event, exercise, metrics):
        ...
        is_major_issue = event in ["workout_started","workout_ended_early", "set_completed", "workout_completed"]

        if not is_major_issue:
            if not issue:
                return None
            
            if now - self.last_spoken_at < 5:     # if not major_issue then we will make sure that it wont speak frequently
                return None
